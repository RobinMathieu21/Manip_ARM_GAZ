...
%     time1 = Data.Gaz(1).timing(i+1,1)-DEBUTDETEC
    ...
    Data.Gaz(SUJET).time(1:l2,1) = round(86400.*str2double(table2array(T(3:end,10))));  %On récupère et convertit le temps
...
debutdetecc=0;
for DEBUTDETEC= [9000]%, 7500,6000,4500,3000,1500,0,-1500]
...
%     
%     y = [0 fix(max(Data.Gaz(SUJET).NMP([1:2000 2640:end],3))+1)]; % current y-axis limits
%     ylim(y)
...
%     path = append(path,'NMP_Lisse_',name);
%     saveas(gcf,path,'png');
